...
/**
 * Agenda public de la Métropole de Lyon, sur OpenAgenda, et sa clé de lecture.
 *
 * La clé est **publique par conception** : OpenAgenda la destine aux
 * applications clientes, en lecture seule. Elle vit donc avec les autres
 * `EXPO_PUBLIC_*` plutôt qu'en secret EAS — et c'est un choix, pas une
 * négligence : un secret EAS est injecté au build mais **pas aux mises à jour à
 * la volée**, qui repartiraient avec une clé vide et un écran mort sans que rien
 * ne l'explique. De toute façon, tout `EXPO_PUBLIC_*` est inliné dans le bundle
 * et s'extrait d'un APK.
 */
export const NEWS_AGENDA_UID = process.env.EXPO_PUBLIC_OPENAGENDA_AGENDA ?? "";
